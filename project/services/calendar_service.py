...
class CalendarManager:
    ...
    def get_available_slots(self, settings: Dict, date: datetime) -> List[TimeSlot]:
        ...
        try:
            ...
            work_start = datetime.strptime(day_hours['start'], '%H:%M').time()
            work_end = datetime.strptime(day_hours['end'], '%H:%M').time()
            
            # Set current_time to start of working hours
            current_time = self.timezone.localize(
                datetime.combine(date.date(), work_start)
            )
            ...
            while current_time.time() <= work_end:
                ...
                # Check if slot is within working hours and doesn't overlap with busy periods
                if slot_end.time() <= work_end and not any(
