...
class CalendarManager:
    ...
    def _is_within_working_hours(self, dt: datetime, working_hours: Dict) -> bool:
        ...
        day_name = dt.strftime('%A').lower()  # Get day name in lowercase
        
        # Convert working_hours keys to lowercase for case-insensitive comparison
        working_hours_lower = {k.lower(): v for k, v in working_hours.items()}
        
        if day_name not in working_hours_lower:
            return False
            
        hours = working_hours_lower[day_name]
    ...
    def get_available_slots(self, settings: Dict, date: datetime) -> List[TimeSlot]:
        ...
        try:
            ...
            # Generate available slots with improved logic
            available_slots = []
                ...
                if self._is_within_working_hours(current_time, working_hours):
                    ...
                    for busy in busy_periods:
                        ...
                        if current_time < busy_end and slot_end > busy_start:
                            ...
                            current_time = busy_end + timedelta(minutes=buffer_time)
                            break
                    
                    if is_available:
                        available_slots.append(TimeSlot(current_time, slot_end))
                        current_time += timedelta(minutes=meeting_duration + buffer_time)
                    elif not any(datetime.fromisoformat(busy['start'].replace('Z', '+00:00')) <= current_time < datetime.fromisoformat(busy['end'].replace('Z', '+00:00')) for busy in busy_periods):
                        current_time += timedelta(minutes=meeting_duration + buffer_time)
                else:
                    # Move to next hour if outside working hours
